...
void reproduce(statelist &state, parmlist &parms, arma::mat &R, uword j, uword s, uword treecount) {
  ...
  state.F(treecount) = sum(kernel1(distance(state, treecount), parms.m(state.S)) % state.I % parms.lamda(state.S)) * state.B(treecount);
...
void die(statelist &state, parmlist &parms, arma::mat &R, uword j, uword s, uword treeindex) {
   state.F -= kernel2(distance(state, j), parms.m(s)) % state.B * state.I(j) * parms.lamda(s);
...
void grow(statelist &state, parmlist &parms, arma::mat &R, uword j, uword s, int i) {
   ...
   state.F(j) = state.F(j) * parms.beta(state.S(j))/parms.beta(s);
   state.F += kernel2(distance(state, j), parms.m(state.S(j))) % state.B * i * (parms.lamda(state.S(j)) - parms.lamda(s));
   state.E = fmax(0, 1 - sum(parms.omega(state.S)) / parms.K);
   R.col(1) = state.E * parms.f(state.S);
   R(j, 2) = parms.d(state.S(j)) + i * parms.alpha(state.S(j)) * (1 - parms.r(state.S(j)));
   R(j, 3) = parms.g(state.S(j));
   R.col(4) = state.F;
   R(j, 5) = i * parms.mu(state.S(j));
   R(j, 6) = i * parms.alpha(state.S(j)) * parms.r(state.S(j));
}
    
void sicken(statelist &state, parmlist &parms, arma::mat &R, uword j, uword s, int i) {
   state.I(j) += 1;
   state.F += kernel2(distance(state, j), parms.m(s)) % state.B * parms.lamda(s);
...
void resprout(statelist &state, parmlist &parms, arma::mat &R, uword j, uword s, int i) {
   ...
   state.F -= kernel2(distance(state, j), parms.m(s)) % state.B * i * parms.lamda(s);
